...
{.pragma: fh, cdecl, importc, discardable.}
...
proc funchook_prepare*(hook: FuncHook, src, dst: pointer): FUNCHOOK_ERROR {.fh.} ## Prepare hooking
...
template prepare*(fh: FuncHook, src, dst: pointer): FUNCHOOK_ERROR = funchook_prepare(fh, src, dst)
template prepare*(fh: FuncHook, src: ptr int, dst: pointer): FUNCHOOK_ERROR = funchook_prepare(fh, cast[pointer](src), dst)
template prepare*(fh: FuncHook, src: ptr int, dst: ptr int): FUNCHOOK_ERROR = funchook_prepare(fh, cast[pointer](src), cast[pointer](dst))
template prepare*(fh: FuncHook, src: pointer, dst: ptr int): FUNCHOOK_ERROR = funchook_prepare(fh, src, cast[pointer](dst))
template install*(fh: FuncHook, flags: int): FUNCHOOK_ERROR = funchook_install(fh, flags)
template uninstall*(fh: FuncHook, flags: int): FUNCHOOK_ERROR = funchook_uninstall(fh, flags)
